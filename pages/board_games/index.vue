...
<template>
  ...
  <div class="grid grid-cols-1 gap-y-5 mt-5" v-if="!boardGameStore.error && !boardGameStore.loading">
    ...
    <div v-if="boardGameStore.myReserves" v-for="board_game in boardGameStore.myReserves.reverse()" class="p-3 flex max-w-[600px] rounded border border-gray-500 border-2">
      <img v-if="board_game.board_game.image_uri" src={{board_game.board_game.image_uri}} alt="">
      <p v-else class="border border-gray-700 bg-gray-500 text-white text-center p-5 select-none">БЕЗ<br>ФОТО</p>
      <div class="ml-3">
...
export default {
  name: "index",
  components: {TakeBoardGameModal, Spinner},
  setup() {
